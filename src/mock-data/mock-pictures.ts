import {Picture} from '../properties/picture';

export const PICTURESOFTHEWEEK: Picture[] = [
    {
		title: "Buxton",
		image: "img/buxton.jpg",
		description: "Walking along the bridge on the Buxton Trail",
		location: "Buxton",
		state: "Oregon",
		week: "July 26, 2015"
	}, {
		title: "Budd Inlet",
		image: "img/budd.jpg",
		description: "Taking a stroll along the Budd Inlet.  Don't worry, that X won't symbolize your death like in The Departed",
		location: "Olympia",
		state: "Washington",
		week: "August 2, 2015"
	}, {
		title: "Netarts",
		image: "img/netarts.jpg",
		description: "Enjoying a nice foggy view at the Oregon Coast.",
		location: "Netarts",
		state: "Oregon",
		week: "August 9, 2015"
	}, {
		title: "Rowena",
		image: "img/rowena.jpg",
		description: "A fantastic view of Mt. Adams and the Columbia River with winds so high it will blow your hat right off!",
		location: "Rowena",
		state: "Oregon",
		week: "August 16, 2015"
	}, {
		title: "Overlook Park",
		image: "img/overlook.jpg",
		description: "Shabbat in the Park: We all Scream for Ice Cream!",
		location: "Portland",
		state: "Oregon",
		week: "August 23, 2015"
	}, {
		title: "St. Johns Bridge",
		image: "img/stjohns.jpg",
		description: "Looking up at the large and in-charge St. John's Bridge",
		location: "Portland",
		state: "Oregon",
		week: "August 30, 2015"
	}, {
		title: "Pipestone National Monument",
		image: "img/pipestone.jpg",
		description: "Throwback pic of Pipestone National Monument in Minnesota.  That was a nice trip.",
		location: "Pipestone National Monument",
		state: "Minnesota",
		week: "September 6, 2015"
	}, {
		title: "View of Eugene",
		image: "img/eugene.jpg",
		description: "Reminiscing of old times in Eugene while seeing new spots.",
		location: "Eugene",
		state: "Oregon",
		week: "September 13, 2015"
	}, {
		title: "Koosah Falls",
		image: "img/koosah.jpg",
		description: "I forgot how wonderful this picture of Koosah Falls was.  Amazing!",
		location: "McKenzie River",
		state: "Oregon",
		week: "September 20, 2015"
	}, {
		title: "Nashville",
		image: "img/nashville.jpg",
		description: "Artwork and skyscraper perfectly aligned in Nashville.  Perfect, like Marcus Mariota.",
		location: "Nashville",
		state: "Tennessee",
		week: "September 27, 2015"
	}, {
		title: "Medford",
		image: "img/medford.jpg",
		description: "A nice picture of Medford, dedicated to those affected by the tragedy at Umpqua CC.",
		location: "Medford",
		state: "Oregon",
		week: "October 4, 2015"
	}, {
		title: "Tilikum Crossing",
		image: "img/tilikum.jpg",
		description: "Tilikum Crossing: Bridge of the People.  Because the Native Chinook never drove cars.",
		location: "Portland",
		state: "Oregon",
		week: "October 11, 2015"
	}, {
		title: "Mt. Rushmore",
		image: "img/rushmore.JPG",
		description: "Mt. Rushmore, beautiful even in the the less celebrated views.",
		location: "Mt. Rushmore National Monument",
		state: "South Dakota",
		week: "October 18, 2015"
	}, {
		title: "Ashwaubenon",
		image: "img/ashwaubenon.jpg",
		description: "Ashwaubenon, WI: Where I once sneaked into someone's yard to take this picture of the Fox River sunrise.  Strange but true!",
		location: "Ashwaubenon",
		state: "Wisconsin",
		week: "October 25, 2015"
	}, {
		title: "Halloween 2015",
		image: "img/halloween.jpg",
		description: "Happy Halloween! My 2015 costume: A Rastafarian with a triple-chin.",
		location: "Beaverton",
		state: "Oregon",
		week: "November 1, 2015"
	}, {
		title: "Boston",
		image: "img/boston.JPG",
		description: "I wish I was in Boston.  Lucky for those who are there.",
		location: "Boston",
		state: "Massachusetts",
		week: "November 8, 2015"
	}, {
		title: "Portland Prays for Paris",
		image: "img/paris.jpg",
		description: "Pray for Paris, Sincerely: Portland.",
		location: "Portland",
		state: "Oregon",
		week: "November 15, 2015"
	}, {
		title: "Monterosso al Mare",
		image: "img/monterosso.JPG",
		description: "Monterosso al Mare, a beautiful spot along Italy's famed Cinque Terre.",
		location: "Monterosso al Mare",
		state: "Italy",
		week: "November 22, 2015"
	}, {
		title: "Duluth",
		image: "img/duluth.jpg",
		description: "Seeing commercials for Duluth Trading underwear made me think of my trip to Duluth, Minnesota.",
		location: "Duluth",
		state: "Minnesota",
		week: "November 29, 2015"
	}, {
		title: "John Day Fossil Beds",
		image: "img/hay.jpg",
		description: "Sometimes the simple things in life are fascinating.",
		location: "John Day Fossil Beds",
		state: "Oregon",
		week: "December 6, 2015"
	}, {
		title: "Happy Hanukkah",
		image: "img/hanukkah.jpg",
		description: "Best Hanukkah Picture Ever...Happy Hanukkah!!!",
		location: "Baltimore",
		state: "Maryland",
		week: "December 13, 2015"
	}, {
		title: "Merry Hodag Christmas",
		image: "img/hodag.jpg",
		description: "And my favorite Christmas Picture...courtesy of the Hodag of Rhinelander, Wisconsin.",
		location: "Rhinelander",
		state: "Wisconsin",
		week: "December 20, 2015"
	}, {
		title: "Grand Canyon Birthday",
		image: "img/sunset.jpg",
		description: "Ending 2015 with my highlight, my 30th Birthday at the Grand Canyon!!!",
		location: "Grand Canyon National Park",
		state: "Arizona",
		week: "December 27, 2015"
	}, {
		title: "Portland, Oregon",
		image: "img/portland.jpg",
		description: "Beginning the New Year in Portland, my hometown.",
		location: "Portland",
		state: "Oregon",
		week: "January 3, 2016"
	}, {
		title: "My Selfie with Abraham Lincoln",
		image: "img/lincoln.jpg",
		description: "My selfie with Abraham Lincoln.  Sweet!",
		location: "Springfield",
		state: "Illinois",
		week: "January 10, 2016"
	}, {
		title: "The Winter Carnival",
		image: "img/winter.jpg",
		description: "Nothing says 'I love Winter' more than a Winter Carnival",
		location: "St. Paul",
		state: "Minnesota",
		week: "January 17, 2016"
	}, {
		title: "Cholla Cacti at Joshua Tree",
		image: "img/joshua.jpg",
		description: "The opposite of a massive Winter Storm...a hot dry desert with little water.",
		location: "Joshua Tree National Park",
		state: "California",
		week: "January 24, 2016"
	}, {
		title: "Mt. Adams",
		image: "img/adams.jpg",
		description: "A beautiful shot of my mountain, Mt. Adams.",
		location: "Trout Lake",
		state: "Washington",
		week: "January 31, 2016"
	}, {
		title: "Bison crossing the road",
		image: "img/buffalo.JPG",
		description: "Hi, we're stuck in traffic, Bison are crossing the road.  Yep, it can happen.",
		location: "Yellowstone National Park",
		state: "Wyoming",
		week: "February 7, 2016"
	}, {
		title: "The magnificent Oneonta Gorge",
		image: "img/oneonta.jpg",
		description: "The Magnificent Oneonta Gorge, even in Black & White",
		location: "Oneonta Gorge",
		state: "Oregon",
		week: "February 14, 2016"
	}, {
		title: "Humuhumunukunukuapua'a Wai'anapanapa",
		image: "img/hawaii.jpg",
		description: "Looking outside my window makes me want to go to the beautiful tropical Hawaii",
		location: "Wai'anapanapa State Park",
		state: "Hawaii",
		week: "February 21, 2016"
	}, {
		title: "The Great UFO Mystery at St. Mark's Basilica",
		image: "img/venice.jpg",
		description: "The great unsolved mystery of the UFO I saw at St. Mark's Basilica.",
		location: "Venice",
		state: "Italy",
		week: "February 28, 2016"
	}, {
		title: "Rainbow at Work",
		image: "img/rainbow.jpg",
		description: "Sometimes it's good to be in the right place at the right time.",
		location: "Beaverton",
		state: "Oregon",
		week: "March 6, 2016"
	}, {
		title: "My Dad's Blue Picture of Dutch Harbor",
		image: "img/blue.jpg",
		description: "My Dad took this picture in Dutch Harbor: I'd feel blue too being here.",
		location: "Dutch Harbor",
		state: "Alaska",
		week: "March 13, 2016"
	}, {
		title: "Palouse Falls",
		image: "img/palouse.jpg",
		description: "Amazing Palouse Falls, I apologize for nothing.",
		location: "Palouse Falls State Park",
		state: "Washington",
		week: "March 20, 2016"
	}, {
		title: "Oregon State Capitol",
		image: "img/salem.jpg",
		description: "Our cherry blossoms are better than D.C.'s.",
		location: "Salem",
		state: "Oregon",
		week: "March 27, 2016"
	}, {
		title: "Cloud Gate",
		image: "img/cloudgate.jpg",
		description: "Nothing says amazing more than Chicago's Cloud Gate.",
		location: "Chicago",
		state: "Illinois",
		week: "April 3, 2016"
	}, {
		title: "Kites at Seaside",
		image: "img/seaside.jpg",
		description: "Having an adventure when you least expect it makes it even better!",
		location: "Seaside",
		state: "Oregon",
		week: "April 10, 2016"
	}, {
		title: "Prauge Castle",
		image: "img/prague.jpg",
		description: "Vítejte na krásné Praze, v České republice!",
		location: "Prague",
		state: "Czech Republic",
		week: "April 17, 2016"
	}, {
		title: "R.I.P. Prince",
		image: "img/first-avenue.jpg",
		description: "R.I.P. Prince",
		location: "Minneapolis",
		state: "Minnesota",
		week: "April 24, 2016"
	}, {
		title: "Two Dogs",
		image: "img/dogs.jpg",
		description: "Two dogs out for a race enjoying life, the way it should be",
		location: "Hood River",
		state: "Oregon",
		week: "May 1, 2016"
	}, {
		title: "Denver",
		image: "img/denver.jpg",
		description: "My birthday present eight years ago: A trip to Denver, fun times!",
		location: "Denver",
		state: "Colorado",
		week: "May 8, 2016"
	}, {
		title: "The Gateway Arch",
		image: "img/gateway.jpg",
		description: "The Gateway Arch: How something so simple can be so Perfect.",
		location: "St. Louis",
		state: "Missouri",
		week: "May 15, 2016"
	}, {
		title: "Lahaina",
		image: "img/lahaina.jpg",
		description: "To help a friend who needs cheering up: A nice picture of a beach in the best place for beaches, Maui.",
		location: "Lahaina",
		state: "Hawaii",
		week: "May 22, 2016"
	}, {
		title: "Monkey Face",
		image: "img/monkey.jpg",
		description: "Look Mom!  I made it to the top of the Monkey Face...I wish.",
		location: "Smith Rock State Park",
		state: "Oregon",
		week: "May 29, 2016"
	}, {
		title: "Moscone Center",
		image: "img/moscone.jpg",
		description: "To honor a friend who's living life happy and free as he should.  I'm proud.",
		location: "San Francisco",
		state: "California",
		week: "June 5, 2016"
	}, {
		title: "Nowhere",
		image: "img/nowhere.jpg",
		description: "What happens in the middle of nowhere, stays in the middle of nowhere.",
		location: "Wheeler County",
		state: "Oregon",
		week: "June 12, 2016"
	}, {
		title: "Mt. Rainer National Park",
		image: "img/rainier-collage.jpg",
		description: "I couldn't choose one Picture of the Week from my day trip to Mt. Rainier, so I chose 16 instead.",
		location: "Mt. Rainier National Park",
		state: "Washington",
		week: "June 19, 2016"
	}, {
		title: "Mirage",
		image: "img/mirage.jpg",
		description: "Summer begins, here's hoping it's not as hot as Las Vegas in July.",
		location: "Las Vegas",
		state: "Nevada",
		week: "June 26, 2016"
	}, {
		title: "The White House",
		image: "img/white-house.jpg",
		description: "Happy 4th of July, from someone who loves America!",
		location: "Washington",
		state: "D.C.",
		week: "July 3, 2016"
	}, {
		title: "Cathedral Caverns",
		image: "img/cathedral-cavern.jpg",
		description: "Rule Number 1 for a Successful Roadtrip, Make sure your car is in good shape.  Learned that in Alabama.",
		location: "Cathedral Caverns State Park",
		state: "Alabama",
		week: "July 10, 2016"
	}, {
		title: "Ars Electronic Center",
		image: "img/ars.jpg",
		description: "The Future for me started in 2005 at the Ars Electronica Center in Linz, Austria",
		location: "Linz",
		state: "Austria",
		week: "July 17, 2016"
	}, {
		title: "Reno",
		image: "img/reno.jpg",
		description: "The Higher the Better: The Story of My Trip to Reno",
		location: "Reno",
		state: "Nevada",
		week: "July 24, 2016"
	}, {
		title: "Crater Lake",
		image: "img/crater.jpg",
		description: "Kinda looks like Lake Tahoe doesn't it?  But no, it's Crater Lake",
		location: "Crater Lake National Park",
		state: "Oregon",
		week: "July 31, 2016"
	}, {
		title: "Columbia River",
		image: "img/columbia.jpg",
		description: "The Columbia River: A Great Place in Oregon to show to your Cousin",
		location: "Portland",
		state: "Oregon",
		week: "August 7, 2016"
	}, {
		title: "Shuttlecocks",
		image: "img/shuttlecock.jpg",
		description: "Everything's better when bigger, including Shuttlecocks",
		location: "Kansas City",
		state: "Missouri",
		week: "August 14, 2016"
	}, {
		title: "Detroit Lake",
		image: "img/detroit-lake.jpg",
		description: "Remember when Detroit Lake was actually filled with water?  I do.",
		location: "Detroit Lake",
		state: "Oregon",
		week: "August 21, 2016"
	}, {
		title: "Cahokia Mounds",
		image: "img/cahokia.jpg",
		description: "Cahokia Mounds: Where Native American history meets big city views",
		location: "Cahokia Mounds",
		state: "Illinois",
		week: "August 28, 2016"
	}, {
		title: "Sunset at Home",
		image: "img/home.jpg",
		description: "Sometimes, the best joys in life are at home.",
		location: "Beaverton",
		state: "Oregon",
		week: "September 4, 2016"
	}, {
		title: "Three Sisters",
		image: "img/sister.jpg",
		description: "I love Mountains.  Central Oregon has plenty great Mountains.",
		location: "Cascade Lakes Scenic Byway",
		state: "Oregon",
		week: "September 11, 2016"
	}, {
		title: "Omaha",
		image: "img/omaha.jpg",
		description: "Respect your Opponent: Some things are bigger than a Game.",
		location: "Omaha",
		state: "Nebraska",
		week: "September 18, 2016"
	}, {
		title: "Winona",
		image: "img/winona.jpg",
		description: "Is this the turning point to the biggest mystery ever? Only time will tell...",
		location: "Winona",
		state: "Minnesota",
		week: "September 25, 2016"
	}, {
		title: "Cascade Locks",
		image: "img/locks.jpg",
		description: "Putting the Locks on Cascade Locks",
		location: "Cascade Locks",
		state: "Oregon",
		week: "October 2, 2016"
	}, {
		title: "Jacksonville",
		image: "img/jacksonville.jpg",
		description: "My thoughts to those affected by Hurricane Matthew.  Jacksonville much nicer when my Mom visited.",
		location: "Jacksonville",
		state: "Florida",
		week: "October 9, 2016"
	}, {
		title: "Ruby Falls Cave.",
		image: "img/ruby.jpg",
		description: "In honor of my cousins from Tennessee visiting, a picture inside Ruby Falls Cave.",
		location: "Ruby Falls",
		state: "Tennessee",
		week: "October 16, 2016"
	}, {
		title: "Cat vs. Mole, the Epic Battle",
		image: "img/cat-vs-mole.jpg",
		description: "Cat vs. Mole, the Epic Battle",
		location: "Beaverton",
		state: "Oregon",
		week: "October 23, 2016"
	}, {
		title: "Double Rainbow",
		image: "img/double-rainbow.jpg",
		description: "If this isn't what life's all about, I don't know what is",
		location: "Beaverton",
		state: "Oregon",
		week: "October 30, 2016"
	}, {
		title: "Reno Arch",
		image: "img/reno-bell.jpg",
		description: "The future of our country could be made here: I held a bell for Red, White and Blue.  Not White, Blue and Red",
		location: "Reno",
		state: "Nevada",
		week: "November 6, 2016"
	}, {
		title: "Pompeii",
		image: "img/pompeii.jpg",
		description: "Pompeii: A perfect analogy to describe what the United States moral values have become with Dictator Trump in charge",
		location: "Pompeii",
		state: "Italy",
		week: "November 13, 2016"
	}, {
		title: "Sheboygan",
		image: "img/sheboygan.jpg",
		description: "Welcome to Wisconsin: Home of the Cheese, Land of the Fat",
		location: "Sheboygan",
		state: "Wisconsin",
		week: "November 20, 2016"
	}, {
		title: "Warm Springs",
		image: "img/warm-springs.jpg",
		description: "Native Americans deserve more respect than they get",
		location: "Warm Springs",
		state: "Oregon",
		week: "November 27, 2016"
	}, {
		title: "Blizzard",
		image: "img/blizzard.jpg",
		description: "Winter's almost here, time to bundle up!",
		location: "Minneapolis",
		state: "Minnesota",
		week: "December 4, 2016"
	}, {
		title: "Ka'anapali",
		image: "img/beach.jpg",
		description: "A crazy week is all you need to wish you were in Hawaii",
		location: "Ka'anapali",
		state: "Hawaii",
		week: "December 11, 2016"
	}, {
		title: "A Car Parts Winter Wonderland",
		image: "img/car-parts.jpg",
		description: "A Car Parts Winter Wonderland",
		location: "Sherwood",
		state: "Oregon",
		week: "December 18, 2016"
	}, {
		title: "34th Street Lights",
		image: "img/holiday.jpg",
		description: "Happy Holidays!",
		location: "Baltimore",
		state: "Maryland",
		week: "December 25, 2016"
	}, {
		title: "Lake Tahoe",
		image: "img/tahoe.jpg",
		description: "Happy New Years!",
		location: "Lake Tahoe",
		state: "California",
		week: "January 1, 2017"
	}, {
		title: "Sunrise in Iowa",
		image: "img/sunrise.jpg",
		description: "Start the New Year off right",
		location: "Worth County",
		state: "Iowa",
		week: "January 8, 2017"
	}, {
		title: "Mt. Hood",
		image: "img/hood.jpg",
		description: "A Snowstorm is not the End of the World",
		location: "Mt. Hood",
		state: "Oregon",
		week: "January 15, 2017"
	}, {
		title: "Top Secret",
		image: "img/top-secret.jpg",
		description: "The White House has officially been turned upside down",
		location: "Wisconsin Dells",
		state: "Wisconsin",
		week: "January 22, 2017"
	}, {
		title: "Shaniko",
		image: "img/shaniko.jpg",
		description: "The Good Things in Life are all that Matters",
		location: "Shaniko",
		state: "Oregon",
		week: "January 29, 2017"
	}, {
		title: "Seattle",
		image: "img/seattle.jpg",
		description: "Bad Ideas are bad ideas: All sides can agree to that",
		location: "Seattle",
		state: "Washington",
		week: "February 5, 2017"
	}, {
		title: "Cape Perpetua",
		image: "img/perpetua.jpg",
		description: "Sometimes it's good to get away from it all",
		location: "Cape Perpetua",
		state: "Oregon",
		week: "February 12, 2017"
	}, {
		title: "Sisters",
		image: "img/sisters.jpg",
		description: "Welcome to the Wild Wild West",
		location: "Sisters",
		state: "Oregon",
		week: "February 19, 2017"
	}, {
		title: "Giraffe",
		image: "img/giraffe.jpg",
		description: "Who Knew a Giraffe could be so Addicting",
		location: "Portland",
		state: "Oregon",
		week: "February 26, 2017"
	}, {
		title: "Clear Lake",
		image: "img/pyramid.jpg",
		description: "Is this where the Egyptians first settled in America?",
		location: "Clear Lake",
		state: "Iowa",
		week: "March 5, 2017"
	}, {
		title: "Chicago",
		image: "img/chicago.jpg",
		description: "Ten Years of Being Awesome",
		location: "Chicago",
		state: "Illinois",
		week: "March 12, 2017"
	}, {
		title: "Potheads, the Innocent Version",
		image: "img/pothead.jpg",
		description: "Potheads, the Innocent Version",
		location: "Silverton",
		state: "Oregon",
		week: "March 19, 2017"
	}, {
		title: "Final Four!",
		image: "img/ducks.jpg",
		description: "Final Four!",
		location: "Eugene",
		state: "Oregon",
		week: "March 26, 2017"
	}, {
		title: "Train Tracks",
		image: "img/train.jpg",
		description: "Going back to the 19th Century",
		location: "Columbia Hills State Park",
		state: "Washington",
		week: "April 2, 2017"
	}, {
		title: "A Pixelated Birdy",
		image: "img/birdy.jpg",
		description: "A Pixelated Birdy in a 3-D World",
		location: "Palm Desert",
		state: "California",
		week: "April 9, 2017"
	}, {
		title: "Cape Lookout",
		image: "img/lookout.jpg",
		description: "Be On The Lookout for Fun",
		location: "Cape Lookout State Park",
		state: "Oregon",
		week: "April 16, 2017"
	}, {
		title: "I Love Science",
		image: "img/science.jpg",
		description: "I Love Science",
		location: "Portland",
		state: "Oregon",
		week: "April 23, 2017"
	},
	{
		title: "Portland Sunset",
		image: "img/portland-sunset.jpg",
		description: "Loving Life and Experiencing it to the Max",
		location: "Portland",
		state: "Oregon",
		week: "April 30, 2017"
	},
	{
		title: "Painted Hills",
		image: "img/paint.jpg",
		description: "Nature is such a Beautiful Thing. Don't let the Orange Man Destroy It.",
		location: "John Day Fossil Beds National Monument",
		state: "Oregon",
		week: "May 7, 2017"
	},
	{
		title: "Behind Middle North Falls",
		image: "img/middle-north.jpg",
		description: "Behind the Waterfall: The Story of my 32nd Birthday",
		location: "Silver Falls State Park",
		state: "Oregon",
		week: "May 14, 2017"
	},
	{
		title: "Sportsmen's Lodge",
		image: "img/pool.jpg",
		description: "Lots of Fun in Los Angeles, made even better with my family.",
		location: "Los Angeles",
		state: "California",
		week: "May 21, 2017"
	},
		{
			title: "Upper Klamath Lake",
			image: "img/free-bird.jpg",
			description: "Like this Bird, I feel Free at Last",
			location: "Klamath Falls",
			state: "Oregon",
			week: "May 28, 2017"
	},
		{
			title: "Save the Earth?  Absolutely!",
			image: "img/earth.jpg",
			description: "Save the Earth?  Absolutely!",
			location: "Crandon",
			state: "Wisconsin",
			week: "June 4, 2017"
	},
		{
			title: "Baby Got Back",
			image: "img/baby.jpg",
			description: "Baby Got Back",
			location: "Laurel",
			state: "Maryland",
			week: "June 11, 2017"
	},
		{
			title: "Baltimore",
			image: "img/baltimore.jpg",
			description: "East Coast, Here I Come!!!",
			location: "Baltimore",
			state: "Maryland",
			week: "June 18, 2017"
	},
		{
			title: "New York City",
			image: "img/new-york.jpg",
			description: "New York City: I came, I saw, I conquered!",
			location: "New York",
			state: "New York",
			week: "June 25, 2017"
	},
		{
			title: "Anne Amie Vinyards",
			image: "img/anne.jpg",
			description: "Make Like Your Own Paradise, One View at a Time",
			location: "Carlton",
			state: "Oregon",
			week: "July 2, 2017"
	},
		{
			title: "Cottonwood Canyon State Park",
			image: "img/cottonwood.jpg",
			description: "A Journey Through Time",
			location: "Wasco",
			state: "Oregon",
			week: "July 9, 2017"
	},
		{
			title: "Boston from Bunker Hill Monument",
			image: "img/bunker.jpg",
			description: "Boston Cream Pies from Above",
			location: "Boston",
			state: "Massachusetts",
			week: "July 16, 2017"
	},
		{
			title: "Burney Falls",
			image: "img/burney.jpg",
			description: "Burney Falls: One Year Later, Still Beautiful",
			location: "McArthur-Burney Falls Memorial State Park",
			state: "California",
			week: "July 23, 2017"
	}, {
			title: "Phoenix",
			image: "img/phoenix.jpg",
			description: "The Maverick Strikes Again!",
			location: "Phoenix",
			state: "Arizona",
			week: "July 30, 2017"
	}, {
			title: "Roman Forum",
			image: "img/rome.jpg",
			description: "Roman Forum: Always Legendary",
			location: "Rome",
			state: "Italy",
			week: "August 6, 2017"
	}, {
			title: "Buck'Em Cowboy",
			image: "img/cowboy.jpg",
			description: "Buck'Em Cowboy",
			location: "Pendleton",
			state: "Oregon",
			week: "August 13, 2017"
	}, {
			title: "Eclipse!!!",
			image: "img/eclipse.jpg",
			description: "The Eclipse is Coming!!!",
			location: "Ashland",
			state: "Oregon",
			week: "August 20, 2017"
	}, {
			title: "The Great Eclipse of 2017",
			image: "img/adam-eclipse.jpg",
			description: "I survived the Great Eclipse of 2017.  Now where's my T-Shirt?",
			location: "Beaverton",
			state: "Oregon",
			week: "August 27, 2017"
	}, {
			title: "Philadelphia Skyline",
			image: "img/philadelphia.jpg",
			description: "The Wait is Almost Here!",
			location: "Philadelphia",
			state: "Pennsylvania",
			week: "September 3, 2017"
	}, {
			title: "Punch Bowl Falls",
			image: "img/punch-bowl.jpg",
			description: "The Columbia River Gorge, A True Jewel",
			location: "Eagle Creek",
			state: "Oregon",
			week: "September 10, 2017"
	}, {
			title: "Go Gophers Go!!!",
			image: "img/gopher.jpg",
			description: "Gophers 48 - Beavers 14.  The picture that says it all...",
			location: "Corvallis",
			state: "Oregon",
			week: "September 17, 2017"
	}, {
		title: "Oracle Arena",
		image: "img/oakland.jpg",
		description: "Don't Hate The Player, Hate the Game: First Amendent Rights For All",
		location: "Oakland",
		state: "California",
		week: "September 24, 2017"
}, {
	title: "Clear Lake",
	image: "img/clear.jpg",
	description: "Peace",
	location: "Clear Lake",
	state: "Oregon",
	week: "October 1, 2017"
}, {
	title: "Pray for Vegas",
	image: "img/mandalay.jpg",
	description: "Pray for Vegas",
	location: "Las Vegas",
	state: "Nevada",
	week: "October 8, 2017"
}, {
	title: "All Hail the Rap God",
	image: "img/eminem.jpg",
	description: "All Hail the Rap God",
	location: "Minneapolis",
	state: "Minnesota",
	week: "October 15, 2017"
}, {
	title: "Idaho State Capitol",
	image: "img/boise.jpg",
	description: "My Private Lincoln in Idaho",
	location: "Boise",
	state: "Idaho",
	week: "October 22, 2017"
}, {
	title: "My Car's En Fuego",
	image: "img/fire.jpg",
	description: "My Car's En Fuego",
	location: "White Salmon",
	state: "Washington",
	week: "October 29, 2017"
}, {
	title: "Hat Rock State Park",
	image: "img/hat.jpg",
	description: "Nice Hat!",
	location: "Hat Rock State Park",
	state: "Oregon",
	week: "November 5, 2017"
}, {
	title: "Iao Valley State Park",
	image: "img/ioa.jpg",
	description: "I Can do Black and White Too: Iao!",
	location: "Ioa Valley State Park",
	state: "Hawaii",
	week: "November 12, 2017"
}, {
	title: "Rochester, Minnesota",
	image: "img/rochester.jpg",
	description: "I was here once, this actually happened.",
	location: "Rochester",
	state: "Minnesota",
	week: "November 19, 2017"
}, {
	title: "Mirror Lake",
	image: "img/mirror.jpg",
	description: "Sometimes it's fun to reminisce about the good times.",
	location: "Mount Hood National Forest",
	state: "Oregon",
	week: "November 26, 2017"
}, {
	title: "New York New York in Las Vegas",
	image: "img/jenn-vegas.jpg",
	description: "One picture from my Mom, one from my Dad, now it's my Sister's turn",
	location: "Las Vegas",
	state: "Nevada",
	week: "December 3, 2017"
}, {
	title: "Lake Winnebago",
	image: "img/winnebago.jpg",
	description: "Next Stop on the Greatest Mystery Ever Told",
	location: "Fond du Lac",
	state: "Wisconsin",
	week: "December 10, 2017"
}, {
	title: "Huntsville",
	image: "img/huntsville.jpg",
	description: "Thank You Alabama: No Moore Hate",
	location: "Hunstville",
	state: "Alabama",
	week: "December 17, 2017"
}, {
	title: "Portland Skyline",
	image: "img/portland-night.jpg",
	description: "Portland, Oregon: My Hometown at Night",
	location: "Portland",
	state: "Oregon",
	week: "December 24, 2017"
}, {
	title: "New York, New York",
	image: "img/liberty.jpg",
	description: "Happy New Year!  Let's make this a Great Orange-Free Year",
	location: "New York",
	state: "New York",
	week: "December 31, 2017"
}, {
	title: "Humpty Trumpty",
	image: "img/humpty.jpg",
	description: "Humpty Trumpty sat on a wall. Humpty Trumpty had a great fall.  All of Trump's horses and all of Trump's Men.  Couldn't put Trumpty together again.",
	location: "Rock City",
	state: "Georgia",
	week: "January 7, 2018"
}, {
	title: "Three Fingered Jack",
	image: "img/three-fingered-jack.jpg",
	description: "The Legend of Three Fingered Jack Blazing the Trail Through the Oregon West",
	location: "Redmond",
	state: "Oregon",
	week: "January 14, 2018"
}, {
	title: "Skol Vikings!",
	image: "img/viking.jpg",
	description: "Skol Vikings! Bring It Home!",
	location: "Alexandria",
	state: "Minnesota",
	week: "January 21, 2018"
}, {
	title: "Case Closed",
	image: "img/minneapolis.jpg",
	description: "Case Closed",
	location: "Minneapolis",
	state: "Minnesota",
	week: "January 28, 2018"
}, {
	title: "Phantom Ship",
	image: "img/ship.jpg",
	description: "Real Ship, Meet Phantom Ship",
	location: "Crater Lake National Park",
	state: "Oregon",
	week: "February 4, 2018"
}, {
	title: "Utah Lake and Mt. Timpanogos",
	image: "img/utah.jpg",
	description: "New Family in Utah.  Awesome!",
	location: "Utah Lake",
	state: "Utah",
	week: "February 11, 2018"
}, {
	title: "Ecola State Park",
	image: "img/ecola.jpg",
	description: "The Beautiful Oregon Coast: One of a Kind",
	location: "Cannon Beach",
	state: "Oregon",
	week: "February 18, 2018"
}, {
	title: "Fire 'n' Ice",
	image: "img/fire-ice.jpg",
	description: "Fire 'n' Ice: A great way to enjoy snow and sun in the morning",
	location: "Beaverton",
	state: "Oregon",
	week: "February 25, 2018"
}, {
	title: "Abandoned Building in Antelope",
	image: "img/antelope.jpg",
	description: "Anything can happen anywhere, even in the smallest of towns",
	location: "Antelope",
	state: "Oregon",
	week: "March 4, 2018"
}, {
	title: "House on Stilts",
	image: "img/columbia-city.jpg",
	description: "How does that House stay up like that?",
	location: "Columbia City",
	state: "Oregon",
	week: "March 11, 2018"
}, {
	title: "Peace Arch",
	image: "img/canada.jpg",
	description: "O Canada!  O Canada!  I made it to Canada!",
	location: "Surrey",
	state: "British Columbia",
	week: "March 18, 2018"
}, {
	title: "Wallowa Lake Tramway",
	image: "img/joseph.jpg",
	description: "Now That's a Mighty Mighty Drop!",
	location: "Joseph",
	state: "Oregon",
	week: "March 25, 2018"
}, {
	title: "San Gorgonio Pass",
	image: "img/wind.jpg",
	description: "The road to Palm Springs: Wind, Wind and More Wind",
	location: "Palm Springs",
	state: "California",
	week: "April 1, 2018"
}, {
	title: "National Civil Rights Museum",
	image: "img/mlk.jpg",
	description: "50 Years Later: Still Tragic",
	location: "Memphis",
	state: "Tennessee",
	week: "April 8, 2018"
}, {
	title: "Bend from Pilot Butte",
	image: "img/bend.jpg",
	description: "Bend, Bend, Oh Wonderful Bend, How Wondeful Bend Could Be",
	location: "Bend",
	state: "Oregon",
	week: "April 15, 2018"
}, {
	title: "Victoria",
	image: "img/victoria.jpg",
	description: "Victoria, the Beautiful!",
	location: "Victoria",
	state: "British Columbia",
	week: "April 22, 2018"
}, {
	title: "Bellingham Sunset",
	image: "img/zuanich.jpg",
	description: "Riding off into the sunset with no Safe Return",
	location: "Bellingham",
	state: "Washington",
	week: "April 29, 2018"
}, {
	title: "Lost Creek Lake",
	image: "img/lost-creek.jpg",
	description: "Lost Tree at Lost Creek Lake",
	location: "Lost Creek Lake",
	state: "Oregon",
	week: "May 6, 2018"
}, {
	title: "Storm Clouds over Baker City",
	image: "img/baker-city.jpg",
	description: "A Hot and Stormy Summer is Coming",
	location: "Baker City",
	state: "Oregon",
	week: "May 13, 2018"
}, {
	title: "Airplane over Stateline",
	image: "img/airplane.jpg",
	description: "Sometimes you just need to get away",
	location: "Stateline",
	state: "Nevada",
	week: "May 20, 2018"
}, {
	title: "Crooked River Ranch",
	image: "img/crooked-river-ranch.jpg",
	description: "Imperfect Yet Perfect: A Great Way to Get Away",
	location: "Crooked River Ranch",
	state: "Oregon",
	week: "May 27, 2018"
}, {
	title: "Times Square",
	image: "img/times-square.jpg",
	description: "Party over Here Until Midnight!!!",
	location: "New York",
	state: "New York",
	week: "June 3, 2018"
}, {
	title: "Horsethief Lake",
	image: "img/horsethief.jpg",
	description: "Mount Hood from Horsethief Butte over Horsethief Lake.",
	location: "Columbia Hills Historical State Park",
	state: "Washington",
	week: "June 10, 2018"
}, {
	title: "Leavenworth",
	image: "img/leavenworth.jpg",
	description: "Leavenworth: Like Germany but in Washington",
	location: "Leavenworth",
	state: "Washington",
	week: "June 17, 2018"
}, {
	title: "Elvis on the Roof",
	image: "img/elvis.jpg",
	description: "And Singing on Rooftop, it's Elvis!",
	location: "Nashville",
	state: "Tennessee",
	week: "June 24, 2018"
}, {
	"title": "Running with the Broncos",
	"image": "img/broncos.jpg",
	"description": "Running with the Broncos towards Denver",
	"location": "Denver",
	"state": "Colorado",
	"week": "July 1, 2018"
}, {
	"title": "Bobbie the Wonder Dog",
	"image": "img/silverton.jpg",
	"description": "Only Great Dogs get Murals",
	"location": "Silverton",
	"state": "Oregon",
	"week": "July 8, 2018"
}, {
	"title": "Hollywood",
	"image": "img/hollywood.jpg",
	"description": "California, here we come!",
	"location": "Hollywood",
	"state": "California",
	"week": "July 15, 2018"
}, {
	"title": "San Clemente",
	"image": "img/san-clemente.jpg",
	"description": "Sunsets are the best when you're with your family",
	"location": "San Clemente",
	"state": "California",
	"week": "July 22, 2018"
}, {
	"title": "LIBRARY!",
	"image": "img/library.jpg",
	"description": "LEARN SOMETHING!",
	"location": "Boise",
	"state": "Idaho",
	"week": "July 29, 2018"
}, {
	"title": "Currin Bridge over Row River",
	"image": "img/bridge.jpg",
	"description": "Covered Bridge to Nowhere",
	"location": "Cottage Grove",
	"state": "Oregon",
	"week": "August 5, 2018"
}, {
	"title": "Yucca brevifolia",
	"image": "img/yucca.jpg",
	"description": "Yucca brevifolia: The Joshua Tree stands alone",
	"location": "Joshua Tree National Park",
	"state": "California",
	"week": "August 12, 2018"
}, {
	"title": "Wooden Shoe Tulip Farm Rainbows",
	"image": "img/tulip.jpg",
	"description": "Tulip Rainbows are the best Man-Made Rainbows",
	"location": "Woodburn",
	"state": "Oregon",
	"week": "August 19, 2018"
}, {
	"title": "Grand Canyon",
	"image": "img/grand-canyon.jpg",
	"description": "R.I.P. John McCain",
	"location": "Grand Canyon National Park",
	"state": "Arizona",
	"week": "August 26, 2018"
}, {
	"title": "Spokane",
	"image": "img/spokane.jpg",
	"description": "I finally made it to Spokane",
	"location": "Spokane",
	"state": "Washington",
	"week": "September 2, 2018"
}, {
	"title": "Lower Lewis River Falls",
	"image": "img/lewis.jpg",
	"description": "Enjoy Summer While it Lasts!",
	"location": "Lewis River",
	"state": "Washington",
	"week": "September 9, 2018"
}, {
	"title": "Stormy Maui",
	"image": "img/storm.jpg",
	"description": "Thinking of those affected by Hurricanes or any other Tropical Storm",
	"location": "Ka'anapali",
	"state": "Hawaii",
	"week": "September 16, 2018"
}, {
	"title": "Mt. Hood Sunrise",
	"image": "img/hood-sunrise.jpg",
	"description": "A Nice View on the Way to Work.",
	"location": "Beaverton",
	"state": "Oregon",
	"week": "September 23, 2018"
}, {
	"title": "Capri Island",
	"image": "img/capri.jpg",
	"description": "Living the High Life in Capri",
	"location": "Capri",
	"state": "Italy",
	"week": "September 30, 2018"
}, {
	"title": "Devil's Homestead",
	"image": "img/devils.jpg",
	"description": "Devil's Anything is always worth checking out!",
	"location": "Lava Beds National Monument",
	"state": "California",
	"week": "October 7, 2018"
}, {
	"title": "Autzen Stadium",
	"image": "img/autzen.jpg",
	"description": "There's No Place Like Autzen Stadium",
	"location": "Eugene",
	"state": "Oregon",
	"week": "October 14, 2018"
}, {
	"title": "Happy Fall!",
	"image": "img/fall.jpg",
	"description": "Happy Fall Y'All!!!",
	"location": "Beaverton",
	"state": "Oregon",
	"week": "October 21, 2018"
}, {
	"title": "Verona Arena",
	"image": "img/verona.jpg",
	"description": "Roses: A True Sign of Love",
	"location": "Verona",
	"state": "Italy",
	"week": "October 28, 2018"
}, {
	"title": "Mt. Hood",
	"image": "img/aerial-hood.jpg",
	"description": "Going on Vacation!  See you in Two Weeks!",
	"location": "Mt. Hood",
	"state": "Oregon",
	"week": "November 4, 2018"
}, {
	"title": "Minnesota > Wisconsin",
	"image": "img/goldy.jpg",
	"description": "Finally, Paul Bunyan's Axe is Ours!!!",
	"location": "Minneapolis",
	"state": "Minnesota",
	"week": "November 25, 2018"
}
]
